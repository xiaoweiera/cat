...
export const getTypeNameByKey = (name: string) => {
  if (name === 'machine_gun_pool_single') {
    return I18n.apy.vaults
  } else if (name === 'lend_single') {
    return I18n.apy.lend
  }
  return I18n.apy.load
}
...
const getChartIndex = (plats: dropList, propsChartIndex: Number) => {
  if (plats.select !== I18n.apy.tagAll && propsChartIndex !== 2) return 0
  if (plats.select === I18n.apy.tagAll && propsChartIndex !== 2) return 1
  return 2
}
// 获取marks和被选择项
const getMarks = (
...
const getAllCoinData = (platsSelect: string, data: any, chain: string) => {
  if (chain === 'ALL') {
    ...
    const item: platInfo = R.find((item: platInfo) => {
      ...
      if (status || platsSelect === I18n.apy.tagAll) {
        return true
      }
    }, data)
...
export const getInfo = (params: dataSetModel, propsChartIndex: number) => {
  ...
  const plats = reactive({
    name: I18n.apy.bigChartFilterType.plat,
    select: '',
    data: [],
  })
  const coins = reactive({
    name: I18n.apy.bigChartFilterType.coin,
    select: '',
    data: [],
  })
  const marks = reactive({
    name: I18n.apy.bigChartFilterType.mark,
    select: '',
    data: [],
  })
  // 得到table数据
