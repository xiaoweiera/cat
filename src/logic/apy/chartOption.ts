...
//获取marks和被选择项
const getMarks = (chartIndex: number, selectName: string, typeSelect: string) => {
    let marks = {data: [], select: ''}
    if (chartIndex === 2 && typeSelect === 'machine_gun_pool_single') {
        //@ts-ignore
        marks.data = ['APY', '用户总收益', 'TVL']
        //@ts-ignore
        marks.select = marks.data.includes(selectName) ? selectName : marks.data[0]
        //@ts-ignore
        return marks
    } else {
        //@ts-ignore
...
//获取平台和首次设置mark
const getPlats = (chartIndex: number, selectName: string, dropData: dropList) => {
    if (chartIndex !== 2) {
        //@ts-ignore
        return dropData.data.includes(selectName) ? selectName : dropData.data[0]
...
const getAllCoinData=(platsSelect:string,data:any,chain:string)=>{
    if(chain==='ALL'){
        let project_name=platsSelect.slice(0,platsSelect.indexOf('('))
        let chain=platsSelect.slice(platsSelect.indexOf('(')+1,platsSelect.indexOf(')')).toLowerCase()
        //@ts-ignore
        return Object.keys(R.find((item: platInfo) => {
            if ((item.project_name === project_name && item.chain===chain) || platsSelect === '全部') {
                return item.data
            }
        }, data).data)
    }else{
...
const getALlPlatsData=(chains:string,data:any)=>{
    if (chains=== 'ALL') {
        return R.map((item: platInfo) => (item.project_name + '(' + item.chain.toLocaleUpperCase() + ')'), data)
    } else {
...
export const getInfo = (params: dataSetModel, propsChartIndex: number) => {
    ...
    const getDataSet = async (params: dataSetModel) => {
        //@ts-ignore
        params.category = types.data.includes(types.select) ? params.category : types.data[0]
    ...
    const getData = async (params: dataSetModel) => {
        ...
        if (propsChartIndex === 1) {
            ...
            //@ts-ignore
            coins.select = coins.data.includes(params.selectedTag) ? params.selectedTag : coins.data[0]
            //@ts-ignore
        ...
        //@ts-ignore 第三种类型图表
        const newMark = getMarks(params.chartIndex, params.selectedTag, types.select)
    ...
    const changeTypes = async (params: dataSetModel) => {
        ...
        coins.data.unshift('全部')
        //@ts-ignore
        coins.select = coins.data.includes(coins.select) ? coins.select : coins.data[0]
        //第三种类型图表
    ...
    //得到图表数据
    const requestFilterData = async (param: dataSetModel, filterName: String, propsChartIndex: Number) => {
        ...
        //@ts-ignore
        const requstData = await tableConfig[tableIndex[param.category]].charts[chartIndex].requestData(reqParam)
        //第一个图表的话就是平台tag，第二个就是币tag
        const tagItem = chartIndex === 0 ? plats.select : (chartIndex === 2 ? marks.select : coins.select)
        //第一个图表的话就能筛选币种，第二个不能，默认一个币种展示全部平台
        filterName = chartIndex === 0 ? (coins.select === '全部' ? '' : coins.select) : (chartIndex === 2 ? (plats.select === '全部' ? '' : plats.select) : '')
        // filterName=chartIndex===0?filterName:(chartIndex===2?(plats.select==='全部'?'':plats.select):'')
        //@ts-ignore
        const data = tableConfig[tableIndex[param.category]].charts[chartIndex].chartData(requstData.data, tagItem)
        // @ts-ignore
        return tableConfig[tableIndex[param.category]].charts[chartIndex].xyData(data, filterName)
    }
    return {chains, types, plats, coins, marks, getData, changeTypes, changePlats, requestFilterData, isChartIndex}
}
