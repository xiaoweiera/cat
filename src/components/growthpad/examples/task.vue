  ...
  <div class="flex p-1.5 task-content items-center" :class="`skin-${skin}`">
    ...
    <div class="ml-1 flex-1 flex justify-between items-center">
      <div class="flex-1 flex justify-between items-center text-sm">
        ...
        <div>
          <span v-if="data.suffix" class="suffix font-kdFang font-bold">{{ data.suffix }}</span>
        </div>
      </div>
      <div class="text-xs ml-1.5">
        <span v-if="data.status" class="button text-center rounded-xl block success">{{ data.success }}</span>
        <a v-else class="button text-center rounded-xl block" :href="data.href">{{ data.wait }}</a>
      </div>
    </div>
  </div>
</template>
...
<style scoped lang="postcss">
  .task-content {
    border-radius: 64px;
    background: linear-gradient(90deg, rgba(43, 141, 254, 0.12) 0%, rgba(43, 141, 254, 0) 100%);
  }
  .description {
