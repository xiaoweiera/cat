...
<template>
  <div>
    <el-table :header-cell-style="getHeaderClass()" :data="tableData" style="width: 100%;border-top:1px solid rgba(3, 54, 102, 0.06);">
      <el-table-column prop="palt" width="170px"  label="平台">
        <template #default="scope">
          <Apy2LoanTablePlat :logo="scope.row.platLogo" :chain="scope.row.platIcon" :name="scope.row.plat" :des="scope.row.platDes"/>
        </template>
      </el-table-column>
      <el-table-column prop="loanAmount" width="170px" label="可借额度">
        <template #default="scope">
          <Apy2LoanTableLoanAmount :value="scope.row.loanAmount"/>
        </template>
      </el-table-column>
      <el-table-column prop="clearNumber"   label="最低抵押率/清算率" sortable >
        <template #default="scope">
          <Apy2LoanTableClearNumber :value0="scope.row.mortRate" :value1="scope.row.clearRate"/>
        </template>
      </el-table-column>
      <el-table-column prop="gain" label="抵押收益" align="center"  sortable>
        <template #default="scope">
          <Apy2LoanTableGain :logo="scope.row.gainLogo" :name="scope.row.gainCoin" :value="scope.row.gain" :des="scope.row.gainDes"/>
        </template>
      </el-table-column>
      ...
      <el-table-column prop="cost" label="真实成本(借出金额)"   sortable>
        <template #default="scope">
         <Apy2LoanTableCost :value="scope.row.cost"/>
        </template>
      </el-table-column>
