...
import {toRefs, watch, defineProps, onBeforeMount, onMounted} from 'vue'
...
const newDraw = async (filterName) => {
  filterName = filterName === '全部' ? '' : filterName
  if (props.beginTime) {
    param = {
      ...param,
  ...
  const {
    ...
    min,
    max,
  } = await requestFilterData(param, filterName, props.chartIndex);
  const unitSelect = isChartIndex === 2 ? plats.select : marks.select
  props.optionData(xData, yData, min, max, tableIndex[types.select], isChartIndex.value, unitSelect)
}
//监听类型
watch(() => types.select, async (n, o) => {
  param.category = n
  changeTypes(param)
  newDraw()
  setDes()
...
const setDes = () => {
  //得到标题
  title.value = tableConfig[tableIndex[types.select]].charts[isChartIndex.value].title
