...
//@ts-ignore
const {selected: selectedTag} = toRefs(props.tagList)
//@ts-ignore
...
<template>
  <div class="flex mt-3 w-full min-h-7 relative  items-center  justify-between ">
    <div class="flex justify-between">
      <template v-for="(item,i) in tagList.platforms">
        <span @click="clickTag(item)" v-if="i<3" :class="selectedTag===item?'selectTag tag hand':'tag hand'">{{
            item
          }}</span>
      </template>
    </div>
    <div v-if="chartIndex!==2 && tagList.platforms.length>3" @mouseenter="showMore()" @mouseleave="closeMore()"
