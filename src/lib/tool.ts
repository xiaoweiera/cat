...
export const tooptipsModel = (item: any, color: any, value: any, unit: string,full:boolean) => {
  ...
  let dom=''
  if(full){
    dom=`<span class="flex flex-1 mr-3 min-w-70" ><p style="font-size:12px;color:#272C33;line-height:1;" class="flex"><img style="margin-bottom:1.5px;width:16px;height:auto;margin-right:5px;" src='${svg}'/> ${info}</p></span>`
  }else{
...
export const toopTipsContainer=(divDoms:string[],full:boolean)=>{
  let domStr=''
  ...
  newDoms.forEach((item,i)=>{
    let str=''
    if(item){
      if(i===0){ str=`<div class="flex border-1" >${item}`}
      else{
        str=i%2===0 ?`</div><div class="flex border-1">${item}`:`${item}`
      }
      domStr+=str
    }
  })
