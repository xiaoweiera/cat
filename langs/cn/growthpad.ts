...
MDEX 以 HECO 生态链为起点，MDEX 会将成功经验不断复制到以太坊、波卡生态，不断拓展 MDEX 生态与社区，让更多 DeFi 用户享受到 MDEX 生态的红利。`,
  ...
  channels: {
    ...
    dashboard: {
      ...
      活动规则：开始任务前需要先登录 KingData 账号并填写用户领取奖励的 HECO 钱包地址，填写完毕后即可根据任务说明选择自己要参加的活动，填写对应的信息进行验证，验证符合条件则给予对应奖励。奖励会在活动结束后统一发放到您登记的地址中；验证持仓价值任务的奖励会发放到您验证持仓的地址中。奖励会在活动结束后 10 个工作日内，统一发放到您登记的地址中；验证持仓价值任务的奖励会发放到您验证持仓的地址中。<span class="text-global-numRed">（所有奖励将以等价值的代币Can进行发放）</span>`
    },
    weibo: {
